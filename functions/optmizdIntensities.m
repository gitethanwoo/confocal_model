function [optimalInt, relativeBright] = excitationCalc(fitLasers, laserIntensity, fluors)
...
relativeBright = laserIntensity.*defaultPower.*EC.*qy.*scaledexFactor/100; %we just multiply all the scalars
optimalInt = laserIntensity./(relativeBright/min(relativeBrightness)); %this just uses the dimmest to scale the laser powers
